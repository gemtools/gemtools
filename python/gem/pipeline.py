...
class PipelineStep(object):
    ...
    def _input(self):
        ...
        return self.pipeline.open_step(self.dependencies[-1])

    def open(self):
        """Open the steps output. The default implementation
        opnes the last file"""
        return gem.files.open(self.files()[-1])
    ...
    def prepare(self, id, pipeline, config):
        ...
        if config["denovo"]:
            ...
            if config["index"] is None or not os.path.exists(config["index"]):
                ...
                create_step_id = pipeline.create_transcriptome("denovo")
...
class MappingPipeline(object):
    ...
    def open_step(self, id):
        """Open the original input files"""
        return self.steps[id].open()
