...
#define GT_BUFFERED_OUTPUT_FILE_CHECK(buffered_output_file) \
  gt_fatal_check(buffered_output_file==NULL|| \
    buffered_output_file->file==NULL||buffered_output_file->file_name==NULL|| \
    buffered_output_file->buffer==NULL,NULL_HANDLER)

#define GT_BUFFERED_OUTPUT_FILE_CONSISTENCY_CHECK(buffered_output_file) \
  GT_BUFFERED_OUTPUT_FILE_CHECK(buffered_output_file); \
  gt_fatal_check( \
    buffered_output_file->buffer_busy>GT_MAX_OUTPUT_BUFFERS|| \
    buffered_output_file->buffer_write_pending>GT_MAX_OUTPUT_BUFFERS,BOF_BUFFER_INCONSISTENCY)

/*
 * Setup
 */
GT_INLINE void gt_buffered_output_setup_buffers(gt_buffered_output_file* const buffered_output_file) {
  /* Output Buffers */
  register uint64_t i;
  for (i=0;i<GT_MAX_OUTPUT_BUFFERS;++i) {
    buffered_output_file->buffer[i]=NULL;
  }
  buffered_output_file->buffer_busy=0;
  buffered_output_file->buffer_write_pending=0;
  /* Mutexes */
  gt_cond_fatal_error(pthread_mutex_init(&buffered_output_file->out_buffer_mutex, NULL),SYS_MUTEX_INIT);
  gt_cond_fatal_error(pthread_cond_init(&buffered_output_file->out_buffer_cond,NULL),SYS_COND_VAR_INIT);
  gt_cond_fatal_error(pthread_mutex_init(&buffered_output_file->out_file_mutex, NULL),SYS_MUTEX_INIT);
  /* Block ID tracer */
  buffered_output_file->mayor_block_id=0;
  buffered_output_file->minor_block_id=0;
}
...
gt_buffered_output_file* gt_buffered_output_stream_new(FILE* file,const gt_output_file_type output_file_type) {
  ...
  /* Output file */
  buffered_output_file->file_name=GT_STREAM_FILE_NAME;
  buffered_output_file->file=file;
  buffered_output_file->file_type=output_file_type;
  /* Setup buffers */
  gt_buffered_output_setup_buffers(buffered_output_file);
...
gt_buffered_output_file* gt_buffered_output_file_new(char* const file_name,const gt_output_file_type output_file_type) {
  ...
  gt_cond_fatal_error(!buffered_output_file,MEM_HANDLER);
  /* Output file */
  buffered_output_file->file_name=file_name;
  gt_cond_fatal_error(!(buffered_output_file->file=fopen(file_name,"w")),FILE_OPEN,file_name); \
  buffered_output_file->file_type=output_file_type;
  /* Setup buffers */
  gt_buffered_output_setup_buffers(buffered_output_file);
  return buffered_output_file;
...
gt_status gt_buffered_output_file_close(gt_buffered_output_file* const buffered_output_file) {
  ...
  // Close file/stream
  gt_cond_error(error_code|=fclose(buffered_output_file->file),FILE_CLOSE,buffered_output_file->file_name);
  // Delete allocated buffers
  register uint64_t i;
  for (i=0;i<GT_MAX_OUTPUT_BUFFERS&&buffered_output_file->buffer[i]!=NULL;++i) {
    gt_output_buffer_delete(buffered_output_file->buffer[i]);
  }
  // Free mutex/CV
  gt_cond_error(error_code|=pthread_mutex_destroy(&buffered_output_file->out_buffer_mutex),SYS_MUTEX_DESTROY);
  gt_cond_error(error_code|=pthread_cond_destroy(&buffered_output_file->out_buffer_cond),SYS_COND_VAR_INIT);
  gt_cond_error(error_code|=pthread_mutex_destroy(&buffered_output_file->out_file_mutex),SYS_MUTEX_DESTROY);
  // Free handler
...
GT_INLINE gt_output_buffer* gt_buffered_output_file_write_buffer(
    gt_buffered_output_file* const buffered_output_file,gt_output_buffer* const output_buffer) {
  GT_BUFFERED_OUTPUT_FILE_CONSISTENCY_CHECK(buffered_output_file);
  register int64_t bytes_written;
  gt_vector* vbuffer = gt_output_buffer_to_vchar(output_buffer);
  GT_BEGIN_MUTEX_SECTION(buffered_output_file->out_file_mutex) {
    bytes_written = fwrite(gt_vector_get_mem(vbuffer,char),1,
        gt_vector_get_used(vbuffer),buffered_output_file->file);
  } GT_END_MUTEX_SECTION(buffered_output_file->out_file_mutex);
  if (gt_expect_true(bytes_written==gt_vector_get_used(vbuffer))) {
...
GT_INLINE gt_output_buffer* gt_buffered_output_file_sorted_write_buffer(
    gt_buffered_output_file* const buffered_output_file,gt_output_buffer* output_buffer) {
  GT_BUFFERED_OUTPUT_FILE_CONSISTENCY_CHECK(buffered_output_file);
  ...
  GT_BEGIN_MUTEX_SECTION(buffered_output_file->out_buffer_mutex) {
    ...
    output_buffer->buffer_state=GT_OUTPUT_BUFFER_WRITE_PENDING;
    victim = (buffered_output_file->mayor_block_id==gt_output_buffer_get_mayor_block_id(output_buffer) &&
              buffered_output_file->minor_block_id==gt_output_buffer_get_minor_block_id(output_buffer));
    if (!victim) {
...
GT_INLINE gt_output_buffer* gt_buffered_output_file_dump_buffer(
    gt_buffered_output_file* const buffered_output_file,gt_output_buffer* const output_buffer) {
  GT_BUFFERED_OUTPUT_FILE_CONSISTENCY_CHECK(buffered_output_file);
  switch (buffered_output_file->file_type) {
    case SORTED_FILE:
      return gt_buffered_output_file_sorted_write_buffer(buffered_output_file,output_buffer);
      break;
    case UNSORTED_FILE:
      return gt_buffered_output_file_write_buffer(buffered_output_file,output_buffer);
      break;
    default:
