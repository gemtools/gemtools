...
GT_INLINE gt_gtf* gt_gtf_read(FILE* input){
	...
	GT_SHASH_BEGIN_ITERATE(gtf->refs,skey,shash_element,gt_gtf_ref) {
		qsort(gt_vector_get_mem(shash_element->entries, gt_gtf_entry*), gt_vector_get_used(shash_element->entries), sizeof(gt_gtf_entry**), (int (*)(void*, void*))gt_gtf_cmp_entries);
