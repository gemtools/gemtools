...
{
  // Insert integer
  register uint64_t* integer = gt_malloc(uint64_t);
  *integer = 9000;
  ...
  // Insert string
  register char* string = gt_malloc(200);
  ...
  register custom_enum* enumeration = gt_alloc(custom_enum);
  *enumeration = VALUE_2;
  ...
  register custom_type* type = gt_alloc(custom_type);
  type->field2 = 2;
...
{
  // Insert integer
  register uint64_t* integer = gt_alloc(uint64_t);
  *integer = 9000;
