...
START_TEST(gt_test_gtf_entry_create)
{
	...
	gt_gtf_entry* e = gt_gtf_entry_new(1, 100, GTF_STRAND_FORWARD, type);

    fail_unless(e->start==1,"Start not set properly");
    fail_unless(e->end==100,"End not set properly");
    fail_unless(e->strand==GTF_STRAND_FORWARD,"Strand not set properly");
...
START_TEST(gt_test_gtf_read)
{
	...
	fail_unless(gt_vector_get_used(ref_2->entries)==5, "Not all chr2 entries created");
...
START_TEST(gt_test_gtf_search)
{
	...
	fclose(fp);
	gt_gtf_ref* ref_1 = gt_gtf_get_ref(gtf, "chr1");
	gt_vector* entries = ref_1->entries;
	uint64_t i = gt_gtf_bin_search(entries, 10);
	fail_unless(i == 0 ,"Search failed for chr1 and 10");
	
	// start
	i = gt_gtf_bin_search(entries, 0);
...
START_TEST(gt_test_gtf_find_matches)
{
	...
	gt_gtf_entry* e;
	gt_gtf_search(gtf, target, "chr1", 1, 100, GTF_STRAND_FORWARD);
	fail_unless(gt_vector_get_used(target) == 0, "Found something :(");

	// find exact
	gt_gtf_search(gtf, target, "chr1", 11874, 12227, GTF_STRAND_FORWARD);
	...
	// find start < s
	gt_gtf_search(gtf, target, "chr1", 1000, 12227, GTF_STRAND_FORWARD);
	fail_unless(gt_vector_get_used(target) == 1, "Found nothing");
	e = *gt_vector_get_elm(target, 0, gt_gtf_entry*);
	fail_unless( e->start == 11874, "Wrong Entry found");
	
	// find start > s
	gt_gtf_search(gtf, target, "chr1", 11900, 12227, GTF_STRAND_FORWARD);
	fail_unless(gt_vector_get_used(target) == 1, "Found nothing");
	e = *gt_vector_get_elm(target, 0, gt_gtf_entry*);
	fail_unless( e->start == 11874, "Wrong Entry found");
	
	// find start > s end < e
	gt_gtf_search(gtf, target, "chr1", 11900, 12200, GTF_STRAND_FORWARD);
	fail_unless(gt_vector_get_used(target) == 1, "Found nothing");
	e = *gt_vector_get_elm(target, 0, gt_gtf_entry*);
	fail_unless( e->start == 11874, "Wrong Entry found");
	
	// find start > s end > e
	gt_gtf_search(gtf, target, "chr1", 11900, 12230, GTF_STRAND_FORWARD);
	fail_unless(gt_vector_get_used(target) == 1, "Found nothing");
	e = *gt_vector_get_elm(target, 0, gt_gtf_entry*);
	fail_unless( e->start == 11874, "Wrong Entry found");
	...
	// find all
	gt_gtf_search(gtf, target, "chr1", 1, 1000000000, GTF_STRAND_FORWARD);
	fail_unless(gt_vector_get_used(target) == 5, "Found nothing");
	
	// find tails
	gt_gtf_search(gtf, target, "chr1", 14409, 69092, GTF_STRAND_FORWARD);
